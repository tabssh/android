...
- **Text that exactly filled a terminal row left a blank line behind it** — the local terminal engine (used by the built-in shell and the serial/telnet consoles) moved to the next row the instant the last column was written, instead of waiting to see whether more text actually followed. A program that fills a row and then sends its own line break got an extra empty row, and the row it had just filled was treated as if the line continued — which also cut long links short. The wrap is now deferred until the next character arrives, matching xterm and the SSH terminal
- **The built-in Tor client never actually shipped** — the native `tor` binaries are fetched from their own release at build time, exactly like the mosh client and the SPICE libraries, but the fetch step was missing from every publishing workflow. Development, beta, and release builds all shipped without it, so the "Built-in Tor" route was silently unavailable no matter what the About dialog's component list said. All three channels now fetch it and fail the build if any ABI is missing it
- **Long links were cut off at the edge of the screen** — tapping a URL that ran past one terminal row (an OAuth/login link, for example) opened only the first row's worth of it, and the underline stopped there too. Links were only rejoined when the terminal itself had wrapped them; full-screen programs that break their own output at the screen width — Claude Code, and anything else drawing its own layout — produced rows the terminal never marked as wrapped, so the rest of the link was dropped. Rows that run to the full width are now rejoined regardless of who broke the line
- **VNC sessions died a fraction of a second after connecting** — the mouse-cursor update the server sends with its first screen update was parsed four bytes short (its image data is preceded by an encoding field the client skipped), so everything read after it came from the wrong offset and the session was torn down as a protocol error. Servers that send an alpha cursor — TigerVNC, libvirt, and anything built on them — were unusable; the console appeared, then went black
- **App lock never asked for the PIN** — with App Lock enabled and a PIN set, launching TabSSH went straight to the connection list; the gate only ran when the separate "Lock when backgrounded" option was also on and the app had previously been backgrounded long enough. The PIN is now required once per app start, and afterwards the background auto-lock timeout behaves as before
- **VNC sessions were also dropped whenever the client asked the server to match the window size** — the resize request was two bytes too long, which corrupted everything the server read after it and made it hang up; and once the resize did go through, the client never told the server that its live-update region had shrunk with the desktop, so the server kept drawing outside the new screen and hung up again. Combined with the cursor fix above, VNC consoles now connect, resize to the device screen, and stay connected
- **The Light theme was unusable** — every screen's app bar rendered as a blank white strip with the title and the back arrow invisible, and the Settings icons were a pale grey that barely showed against the background. App bars now keep their branded blue background in both themes (including the screens that previously drew their own), and every settings icon is a themed vector that follows the current theme's text color
- **The PIN lock screen looked nothing like the rest of the app** — its input box and buttons were plain platform widgets rather than the app's Material styling, so the lock screen rendered with the wrong colors and shapes
- **The "Scrollback Buffer" setting did nothing** — every terminal tab was created with a fixed 2,000-line history regardless of the setting, and the row itself displayed the stored default as a bare "-1". New tabs, split panes, Tasker-launched tabs, and restored sessions now use the configured value (unlimited is capped at 50,000 lines per tab, which is also the new accepted maximum), and the setting reads "Unlimited (capped at 50,000 lines per tab)" or "N lines"
- **"Browse Files (SFTP)" bounced straight back to the connection list unless the host was already open in a terminal** — the file browser only looked for an SSH session that was already running, so opening it directly from a saved connection closed the screen again immediately. It now opens the SSH session itself when one isn't running, and the "+" button for a second SFTP pane lists every saved connection instead of only the ones already connected
- **TigerVNC servers failed with "Unable to parse TLS packet header"** — TigerVNC's default VeNCrypt sub-types (TLSNone/TLSVnc/TLSPlain) secure the session with anonymous Diffie-Hellman, and Android's TLS provider ships no anonymous cipher suites, so committing to VeNCrypt guaranteed a dead session. TabSSH now checks the device for an anonymous suite before choosing VeNCrypt: when there is none and the server also offers a security type it can complete, it connects with that instead and records the downgrade in the application log; VeNCrypt with X509 certificates is unaffected, and a server offering nothing but certificate-less TLS now reports exactly that instead of a TLS parse error
- **VNC and SPICE consoles opened to a permanently black screen** — the terminal activity's reattach path counted only SSH tabs when deciding whether to build its page adapter, so a session started from the VNC host list (which creates the console tab first, then launches the activity) looked like "no tabs", the console page was never bound, and the RFB handshake — which only starts when that page binds — never ran. The connection itself was fine the whole time; the picture simply never arrived. Console tabs also now write their own lines to the application log: console ready with the desktop name and resolution, closed-by-server, and ended-after-rejected-resize
- **"Enter container" opened a plain shell on the Docker host instead of the container** — the ephemeral exec profile inherits the linked connection's settings, including Mosh mode. With Mosh on "auto" (the default) the tab handed off to mosh-server, which always starts a login shell and silently discarded the `docker exec` command, so the terminal landed on the host. Any profile with a RemoteCommand set — container exec tabs, forced-`command=` jails, SFTP-only accounts — now stays on the SSH exec channel and never hands off to Mosh
...
- **Docker management UI reworked around frequency of use** — the host manager's tabs are now ordered Containers, Stacks, Images, Volumes, Networks, Dashboard (the things you act on daily first, glanceable engine info last), and the toolbar menu puts Refresh and Reconnect ahead of the destructive prune actions. Container rows grew a visible action strip — a state-aware start/stop button, one-tap Logs (deep-links straight to the detail Logs tab), one-tap Terminal (`docker exec`, running containers only), and an overflow button — so the core actions no longer hide behind a long-press; logs and exec went from 3 taps to 1. Every long-press/overflow menu across Docker (containers, stacks, images, volumes, networks, hosts) is now a Material bottom sheet with icons, ordered most-used-first with destructive actions separated below a divider and tinted with the error color. Host rows have the same overflow button (Open / Test connection / Edit / Delete). Connection failures on the manager screen and on each list tab now show an inline error message with a Retry button instead of a dialog over a blank screen, and container status dots use the theme's semantic colors
