...
| C  | `make check` after the Settings-IA, drawer/chrome, manifest-label, formatter-consolidation and i18n passes merged; per-screen `screen+key` diff of `res/xml/preferences_*.xml` before vs after; duplicate-name scan and XML parse over the seven-agent `strings.xml` merge; missing-`import io.github.tabssh.R` scan across every modified `.kt` | BUILD SUCCESSFUL in 26m16s, 932 tests, 0 failures, 0 errors, 10 skipped, `lintDebug` clean. Preference diff: 142 keys byte-identical, 0 removed, 28 added (all value-less `PreferenceCategory` keys), and the `screen+key` diff is empty — no preference changed screens, so no upgrade migration is needed. 0 duplicate `<string>`/`<plurals>` names; `strings.xml` parses. Two gate runs: the first failed on `TranscriptViewerActivity` missing `import io.github.tabssh.R`, after which every modified file was swept for that same defect class |
