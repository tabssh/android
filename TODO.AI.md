...
| B-10 | Mosh broken by B-5 — the new `else` branch in `onDisconnected()` fires asynchronously on main thread after `tab.disconnect()` clears `connection=null` but after `connectMosh()` has already set state CONNECTED; clobbers mosh state back to DISCONNECTED. | ✅ `pending` | `SSHTab.kt` |
| B-11 | Hamburger menu missing on large screens — `applySidebarMode()` locks drawer open (`LOCK_MODE_LOCKED_OPEN`) and hides toggle indicator. Auto-open on launch is bad UX. Remove forced sidebar; use normal overlay mode on all screen sizes so hamburger is always visible and drawer starts closed. | ✅ `pending` | `MainActivity.kt` |
