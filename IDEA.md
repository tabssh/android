...
- `TABSSH_SYNC_V2` sync wire format — AES-256-GCM + Argon2id key derivation
  (`sync/encryption/SyncEncryptor.kt`); Desktop and Web read/write the same
  encrypted blob so a device can sync against any storage provider regardless
  of which app last wrote to it
- QR pairing payload — CBOR + AES-256-GCM + Argon2id, `PairingDecryptor.kt` is
  authoritative; a QR generated by Desktop or Web must decode here unmodified,
  and vice versa via `ImportFromQrActivity`
- Backup archive format — `BACKUP_VERSION = 3` (`backup/BackupManager.kt`),
  `{"v":3,"items":[...]}` per entity file; a backup made by any sibling must
  restore cleanly on the others
- Room schema (currently v27, `storage/database/TabSSHDatabase.kt`) — Desktop's
  SQLite and Web's schema track Android's entity shapes and field set; schema
  version numbers are independent per platform but the fields must line up
- Built-in terminal theme catalogue (23 themes, `BuiltInThemes.kt`) — byte-
  identical theme definitions and exported theme JSON across all three
