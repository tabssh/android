...
- **Default-app-role exception:** apps requesting a system default-app role (dialer, SMS, browser, assistant, home/launcher) via `RoleManager.createRequestRoleIntent(ROLE_*)` use a different grant mechanism than the dialog-based flow above — a one-time system role picker, after which holding the role auto-grants the permissions it implies (e.g. `ROLE_DIALER` implies call-log/phone-state access) with no separate per-permission prompt. The mechanism differs; the discipline doesn't — show an in-app rationale before launching the role-request intent, and keep the app usable with the role's feature disabled if the user declines. Plain AOSP `RoleManager` API, no Play Services tie, no F-Droid anti-feature.
- **Accessibility-service-class exception:** apps whose core, declared feature depends on `BIND_ACCESSIBILITY_SERVICE` (screen readers, gesture/automation tools, remote-assist apps) request it via a Settings deep link (`ACTION_ACCESSIBILITY_SETTINGS`), not a runtime dialog — the same rationale-before-ask discipline still applies. Declare only the capability flags the feature actually uses (`canRetrieveWindowContent`, `canPerformGestures`, `canRequestFilterKeyEvents`, etc.), each justified in IDEA.md same as any permission — an unused flag here is a bigger liability than an unused runtime permission, since it widens what the service can observe/control system-wide, not just what one feature can do. Plain AOSP API, no Play Services tie, no F-Droid anti-feature.
- **Launcher-class exception:** apps whose core, declared feature is replacing the home screen (`CATEGORY_HOME`) need `QUERY_ALL_PACKAGES` to enumerate installed apps for the app grid/drawer — `PackageManager` queries are otherwise scoped to declared `<queries>` elements since API 30, which can't express "every launchable app on the device." Prefer a declared `<queries>` block targeting `ACTION_MAIN`/`CATEGORY_LAUNCHER` first; fall back to `QUERY_ALL_PACKAGES` only when the feature genuinely needs visibility beyond launchable apps (e.g. listing non-launchable installed packages too), and justify it in IDEA.md as the launcher-class exception. No Play Services tie, no F-Droid anti-feature (Play separately requires a declared-use form — informative, not the compliance gate here).
- **Autofill-service-class exception:** password-manager/autofill apps request `BIND_AUTOFILL_SERVICE` via `ACTION_REQUEST_SET_AUTOFILL_SERVICE` — another Settings-mediated grant, not a runtime dialog. Pair it with `FLAG_SECURE` on every screen that renders a stored credential; the underlying secret store still follows PART 6's secret-storage rules regardless of this exception. Plain AOSP API, no Play Services tie, no F-Droid anti-feature.
- **Companion-device-class exception:** apps that pair with a specific external device (wearables, BLE peripherals, smart-home hubs) via `CompanionDeviceManager.associate()` get an OS-level exemption most direct-scan implementations miss — once a device is associated, subsequent BLE scans tied to that association skip `ACCESS_FINE_LOCATION` entirely, unlike a raw `BluetoothLeScanner` scan (which needs it pre-API 31, and `BLUETOOTH_SCAN` with `usesPermissionFlags="neverForLocation"` on API 31+ when the scan doesn't derive location). Prefer the CDM association flow over a location-gated scan for this class of app: it's narrower (one named device, not "see all nearby BLE"), it's a plain AOSP API, and it avoids requesting a permission the feature doesn't conceptually need.
- **Health-Connect-class exception:** health/fitness apps whose core, declared feature reads or writes health data use the Health Connect API (`androidx.health.connect`, an on-device AOSP data store, not a Play Services API) instead of the runtime-permission model above — each data type (steps, heart rate, sleep, etc.) is its own granular permission requested through Health Connect's own consent screen. Justify each requested data type in IDEA.md same as any permission; request lazily per data type with a graceful per-type denial path, never an all-or-nothing gate. Document in IDEA.md that Health Connect itself must be present (pre-installed on modern AOSP/most ROMs; where absent, prompt the user to install it from any source — availability varies by ROM, unlike a Play Services dependency this is a missing on-device component, not a missing service).
...
| `phoneCall` | Active VoIP/SIP call (VoIP/calling-class apps) | Telecom framework, self-managed `ConnectionService` + `MANAGE_OWN_CALLS`; system call-style UI (incoming/ongoing) instead of a hand-built notification. Plain AOSP Telecom API — no Play Services dependency, no F-Droid anti-feature |
...
## VPN & firewall apps (optional)

- **VPN/firewall-class exception:** apps whose core, declared feature is intercepting or filtering the device's own traffic (VPN clients, DNS-over-HTTPS/ad-blocking firewalls, per-app traffic monitors) request `BIND_VPN_SERVICE` via `VpnService.prepare()` — a user-confirmed system dialog, not the runtime-permission flow in PART 2. `VpnService` runs as its own foreground service (`dataSync` type, PART 8) with the always-on system notification Android itself supplies for active VPNs — never suppress, hide, or replace that notification with a custom one. Point the user at the OS "Always-on VPN" / "Block connections without VPN" settings instead of reimplementing a kill-switch. A local loopback proxy (e.g. for DNS filtering) binds `127.0.0.1` only, never a routable address. Plain AOSP `VpnService` API, no Play Services tie, no F-Droid anti-feature — unlike bundling a proprietary VPN SDK, which would be.
