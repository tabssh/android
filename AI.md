...
- **Long-press → terminal bottom sheet menu** (`showTerminalMenu()` in `TabTerminalActivity`): slides in from the bottom (always fully on-screen regardless of where the long-press occurred). Contains: New Tab, Open Tabs list, Toggle Keyboard, Toggle Key Bar, Enable/Disable Prefix Key, Find in Scrollback, Snippets, Broadcast, Port Forwarding, Share Session, Close Tab, Disconnect All, Settings.
