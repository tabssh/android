...
- **Long-press → terminal bottom sheet menu** (`showTerminalMenu()` in `TabTerminalActivity`): slides in from the bottom (always fully on-screen regardless of where the long-press occurred). Contains: New Tab, Open Tabs list, Toggle Keyboard, Toggle Key Bar, Find in Scrollback, Snippets, Start/Stop Recording, Broadcast, Port Forwarding, Share Session, Close Tab, Disconnect All, Settings. (Enable/Disable Prefix Key is **not** here — it lives on the PREFIX key's own long-press picker, see the `PREFIX` row in the custom-keyboard table above.) The **Start/Stop Recording** button (`btn_toggle_recording`) relabels itself each time the sheet opens to match the active tab's actual `sessionRecorder?.isRecording()` state, and calls the same `toggleRecording()` used by the action-bar overflow item and the keyboard's STOP_RECORDING key — so all three entry points stay in sync.
- **Text selection ActionMode** (`startTerminalSelectionActionMode`): `TYPE_FLOATING` bar with Copy / Select All / Paste / Cancel. Entered via **"Select Text…" in the clipboard menu** + drag, or double-tap, or `beginWordSelectionAtTouch()`. The ActionMode is a distinct path from the long-press menu — they do not combine by design, but `GestureDetector` schedules a long-press timeout on every `ACTION_DOWN` including the second tap of a double-tap; if that tap was held past the timeout before lifting, `onLongPress` would fire for the same touch-down right after `onDoubleTap`, opening the context menu on top of the selection. `TerminalGestureListener` tracks the down-event time of the last `onDoubleTap` (`lastDoubleTapDownTime`) and ignores an `onLongPress` whose `e.downTime` matches it. During the ActionMode `viewPager?.isUserInputEnabled = false` to prevent accidental tab swipes; restored in `onDestroyActionMode()`.
...
| `PREFIX` | **PRE** / **[PRE]** | Sends the current multiplexer prefix bytes to the terminal **immediately on tap** — PRE is a shortcut for physically pressing the bind (e.g. Ctrl-B), so it acts like that keypress rather than waiting for the user's next key. Label is always `"PRE"`; shows `"[PRE]"` while a lightweight visual-only latch is armed, cleared automatically on the user's next keystroke (tmux/screen is server-side waiting for that keystroke regardless of app state). Color: green when a multiplexer is detected (active state), grey when none. Second tap on PRE while armed cancels the visual latch without resending. **Tap only** is a no-op when the user turns off "Enable PRE Key" (`PreferenceManager.isPrefixKeyEnabled()`, default on; toggleable only from the long-press picker — the redundant Settings → Connection → Multiplexer switch was removed since the picker's toggle is the single source of truth). If no multiplexer is detected, tap shows `showMultiplexerPickerDialog()` with the user's **actual configured prefix** for each type (read from `PreferenceManager.getMultiplexerPrefix()` + `prefixToShortLabel()`), not hardcoded defaults. **Long-press always works, regardless of the enable/disable state** — it opens `showMultiplexerPickerDialog()`, which lists, in order: Auto (detect), tmux (bind), zellij (bind), screen (bind), Off (this connection), Enable/Disable PRE Key toggle — so a disabled PRE key can always be re-enabled from long-press. A picked entry (other than the global toggle) is **persisted as a per-connection override** (`ConnectionProfile.multiplexerOverride`, applied via `SSHTab.applyMultiplexerOverride()` + `persistMultiplexerOverride()`): NULL = auto (detection decides), a type pins the PRE key (detection probes never overwrite it and the periodic probe loop is skipped), `"off"` dims the PRE key for that connection only. Precedence: **override > live detection > global default** (`gesture_multiplexer_type`). A pinned type also drives the profile's multiplexer auto-launch in place of the global default. The same override is editable in ConnectionEditActivity ("PRE Key Override" dropdown in the multiplexer card). |
...
`storage/database/TabSSHDatabase.kt` — **version 7**, schema exported to `app/schemas/` (`3.json`–`7.json`).

Version numbering was deliberately **reset to 3** by commit `0bd35d42` — the v3 release was the intentional last destructive wipe, and every installed build since starts from the v3 baseline. Registered migrations: `MIGRATION_3_4` (adds `vnc_hosts.keep_alive_in_background`), `MIGRATION_4_5` (sync tombstone/shadow tables), `MIGRATION_5_6` (VNC-tab session fields), `MIGRATION_6_7` (adds `connections.multiplexer_override` — per-connection PRE-key pin, NULL = auto). `fallbackToDestructiveMigrationFrom(1, 2)` covers only pre-reset versions that never shipped with persisted user data. Historical `vNN` references elsewhere in this document that don't match the current 3–7 chain's contents (pre-reset v7, v11, v27, v33, v33→34) refer to the **pre-reset numbering** and describe when a column/table was introduced historically, not the current chain.

### 8.2 Entities (21)

| Entity | Table | Notable fields | File |
|---|---|---|---|
| `ConnectionProfile` | `connections` | `id` (UUID), `name`, `host`, `port`, `username`, `authType`, `keyId`, `identityId`, `groupId`, `theme`, jump-host fields, port-knock fields, mosh, multiplexer mode, `multiplexerOverride` (PRE-key pin: NULL auto / type / `"off"`), post-connect script, font override, sync metadata | `entities/ConnectionProfile.kt` |
...
`VncHostsActivity`/`VncHostEditActivity` manage `VncHost` rows (`vnc_hosts` table, §8) for VNC servers reached directly (not via a hypervisor console proxy) — a `Tab.Vnc` inside `TabTerminalActivity` opens a raw socket to `host:port` (or `host:5900+displayNumber`) and drives it with the same `RfbClient` used for hypervisor consoles.
...
- **Wired into swipeable tabs (shipped)** — VNC and hypervisor-console connections join the SSH `ViewPager2`/`TabManager`/`TerminalPagerAdapter` swipe system (§6.3) via a unified `Tab` sealed type (`Tab.Ssh(SSHTab)` / `Tab.Vnc(VncTab)` / `Tab.Console(ConsoleTab)`) instead of SSH-only `SSHTab` lists. `TerminalPagerAdapter` gains `getItemViewType()` (SSH vs VNC vs console) and a `VncViewHolder` alongside the existing `TerminalViewHolder`. The existing `attachEdgeSwipeGate()`/`swipeSuspendedForSelection` mechanism (built for SSH text-selection vs. swipe) generalizes to also suspend swipe when the active tab is graphical (VNC, or a console tab that has switched to graphical mode) and the touch starts inside the content area rather than the edge strip. `VncHostsActivity` remains the VNC host management (CRUD) screen — the same role the SSH connection-profile list plays outside `TabTerminalActivity`. `ProxmoxManagerActivity`/`XCPngManagerActivity` open/activate a `Tab.Console` inside `TabTerminalActivity` instead of launching a standalone viewer. `VMConsoleActivity` (and its now-orphaned `VncStreamHolder` handoff) has been deleted — every caller (`VncHostsActivity`, `LibvirtManagerActivity`, `ProxmoxManagerActivity`, `XCPngManagerActivity`, `MainActivity`'s VNC quick-connect) creates a `Tab.Vnc`/`Tab.Console` on the shared `TabManager` and focuses it via `TabTerminalActivity.EXTRA_TAB_ID`.
- **Background-parking parity for tabs** — `TabManager.parkBackgroundSessions()`/`reclaimBackgroundSessions()` reuse `VncBackgroundSessionStore` + `VncKeepAliveService` for `Tab.Vnc`/graphical `Tab.Console` sessions, called from `TabTerminalActivity.onStop()`/`onResume()`. Unlike `VMConsoleActivity`'s single-session ownership handoff, tabs are Application-scoped and keep their own `rfbClient` reference the whole time — parking here only pauses the client and registers it with the store/service so the OS doesn't reclaim the process while backgrounded; nothing is torn down or unwired. `TabManager` tracks which store keys it parked so a reclaim can distinguish "never parked" from "parked, then idle-swept" and surface the latter as disconnected. Respects `VncHost.keepAliveInBackground` for persisted `VncTab`s; ephemeral `VncTab`s and all `ConsoleTab`s default to parked.
...
- Type-specific manager activities: `ProxmoxManagerActivity`, `XCPngManagerActivity`, `VMwareManagerActivity`, `OciManagerActivity`, `LibvirtManagerActivity`. They show VM/instance lists with power/snapshot/backup actions and open a `Tab.Console`/`Tab.Vnc` inside `TabTerminalActivity` for serial console (OCI has no console — deferred). All four `HypervisorEditActivity`/manager UIs are normalized: `EXTRA_HYPERVISOR_ID`, shared `item_hypervisor_vm.xml` card, inline action buttons (state shown as colored text — Running/Stopped/Paused/Restarting; Hard Reset confirms via dialog).
