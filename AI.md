...
- **Wired into swipeable tabs (decided design, in progress)** — VNC connections join the SSH `ViewPager2`/`TabManager`/`TerminalPagerAdapter` swipe system (§6.3) via a unified `Tab` sealed type (`Tab.Ssh(SSHTab)` / `Tab.Vnc(VncTab)`) instead of SSH-only `SSHTab` lists. `VncTab` reuses `VncBackgroundSessionStore` for pause/resume across activity recreation — no new persistence layer. `TerminalPagerAdapter` gains `getItemViewType()` (SSH vs VNC) and a `VncViewHolder` alongside the existing `TerminalViewHolder`. The existing `attachEdgeSwipeGate()`/`swipeSuspendedForSelection` mechanism (built for SSH text-selection vs. swipe) generalizes to also suspend swipe when the active tab is VNC and the touch starts inside the content area rather than the edge strip. `VncHostsActivity` remains the VNC host management (CRUD) screen — the same role the SSH connection-profile list plays outside `TabTerminalActivity`. `VMConsoleActivity` as a standalone viewer is retired: connecting to a VNC host or hypervisor console now opens/activates a VNC tab inside `TabTerminalActivity`, exactly like SSH profiles do today. Implementation tracked in `TODO.AI.md`.
