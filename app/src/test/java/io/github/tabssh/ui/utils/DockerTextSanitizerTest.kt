 ...
 */
class DockerTextSanitizerTest {

    @Test
    fun `display strips C0 controls and escape sequences`() {
        val raw = "web\u001B[31mprod\u0007"
        val out = DockerText.display(raw)
    ...
    fun `display strips C1 controls and DEL`() {
        val out = DockerText.display("a\u0085b\u009Bc\u007Fd")
        assertEquals("abcd", out)
    }

    @Test
    fun `display collapses line breaks to spaces`() {
        assertEquals("one two three", DockerText.display("one\ntwo\tthree"))
    }

    @Test
    fun `display trims surrounding whitespace`() {
        assertEquals("name", DockerText.display("  name\n"))
    }

    @Test
    fun `display caps at the requested length with an ellipsis`() {
        val out = DockerText.display("x".repeat(400), 16)
    ...
    fun `display returns empty for null and empty input`() {
        assertEquals("", DockerText.display(null))
        assertEquals("", DockerText.display(""))
    }

    @Test
    fun `display leaves ordinary unicode intact`() {
        assertEquals("café-日本", DockerText.display("café-日本"))
    }

    @Test
    fun `block preserves newlines but drops controls`() {
        assertEquals("line1\nline2\n", DockerText.block("line1\n\u001B[2Jline2\n"))
    }

    @Test
    fun `block caps oversized input`() {
        val out = DockerText.block("a".repeat(100), 10)
    ...
    fun `block returns empty for null input`() {
        assertEquals("", DockerText.block(null))
    }
}
