...
class DockerCliParsersTest {
    ...
    @Test
    fun `parses the docker ps label list, keeping values that contain equals signs`() {
        val fixture = """
            {"ID":"abc123def456","Image":"nginx:1.27","Names":"media-web-1","State":"running","Labels":"com.docker.compose.project=media,com.docker.compose.service=web,com.docker.compose.project.working_dir=/srv/media,broken"}
        """.trimIndent()

        val rows = DockerCliParsers.parseNdjson(fixture, DockerCliParsers::parseContainerLine)

        assertEquals(1, rows.size)
        assertEquals("media", rows[0].labels["com.docker.compose.project"])
        assertEquals("web", rows[0].labels["com.docker.compose.service"])
        assertEquals("/srv/media", rows[0].labels["com.docker.compose.project.working_dir"])
        assertEquals(3, rows[0].labels.size)
    }

    @Test
    fun `a container without labels parses to an empty label map`() {
        val fixture = """
            {"ID":"abc123def456","Image":"redis:7","Names":"cache","State":"running","Labels":""}
        """.trimIndent()

        val rows = DockerCliParsers.parseNdjson(fixture, DockerCliParsers::parseContainerLine)

        assertTrue(rows[0].labels.isEmpty())
    }
