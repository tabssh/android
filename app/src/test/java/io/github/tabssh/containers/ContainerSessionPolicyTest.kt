...
import io.github.tabssh.containers.ContainerSessionPolicy.CacheEntry
 ...
 * overflow (see ContainerSessionManager.MAX_OPEN_SESSIONS / IDLE_TIMEOUT_MS).
 */
class ContainerSessionPolicyTest {
    ...
    fun `live fresh sessions under cap are kept`() {
        ...
        assertTrue(ContainerSessionPolicy.selectVictims(entries, 1_000_000L, 16, idle).isEmpty())
    }

    @Test
    fun `dead sessions are always evicted`() {
        val entries = listOf(entry(1, 0), entry(2, 0, connected = false))
        assertEquals(setOf(2L), ContainerSessionPolicy.selectVictims(entries, 1_000_000L, 16, idle))
    }

    @Test
    fun `idle sessions past the timeout are evicted`() {
        val entries = listOf(entry(1, idle + 1), entry(2, idle - 1))
        assertEquals(setOf(1L), ContainerSessionPolicy.selectVictims(entries, 1_000_000L, 16, idle))
    }
    ...
    fun `LRU overflow evicts least recently used first`() {
        ...
        assertEquals(setOf(1L, 2L), ContainerSessionPolicy.selectVictims(entries, 1_000_000L, 2, idle))
    }

    @Test
    fun `keepHostId is exempt from capacity eviction`() {
        val entries = listOf(entry(1, 4000), entry(2, 3000), entry(3, 2000))
        val victims = ContainerSessionPolicy.selectVictims(entries, 1_000_000L, 2, idle, keepHostId = 1L)
    ...
    fun `keepHostId is still evicted when dead`() {
        ...
        val victims = ContainerSessionPolicy.selectVictims(entries, 1_000_000L, 16, idle, keepHostId = 1L)
    ...
    fun `empty cache selects nothing`() {
        assertTrue(ContainerSessionPolicy.selectVictims(emptyList(), 0L, 16, idle).isEmpty())
    }
}
