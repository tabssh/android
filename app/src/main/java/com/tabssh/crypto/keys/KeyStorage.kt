...
class KeyStorage(private val context: Context) {
    ...
    suspend fun generateKeyPair(
        ...
        try {
            ...
            // Create key name
            val keyName = "Generated ${keyType.name} Key"
    ...
    private fun parseTraditionalKey(keyContent: String, passphrase: String?, algorithm: String): ParseResult {
        return try {
            ...
            ParseResult.Success(KeyPair(publicKey, privateKey), "")
    ...
    private fun parsePKCS8Key(keyContent: String, passphrase: String?): ParseResult {
        return try {
            ...
            val privateKey = if (passphrase != null) {
                // Encrypted PKCS8 - would need proper decryption
                Logger.w("KeyStorage", "Encrypted PKCS8 not fully implemented")
                return ParseResult.Error("Encrypted PKCS8 keys not yet supported")
            } else {
                // Unencrypted PKCS8
                val keySpec = PKCS8EncodedKeySpec(keyBytes)
            ...
            ParseResult.Error("Failed to parse PKCS8 key: ${e.message}")
        }
    }
    ...
    private fun formatPublicKey(publicKey: PublicKey, keyType: String, comment: String?): String {
        ...
        return "$keyTypeStr $keyData$commentStr"
    }
}
