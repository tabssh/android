...
/**
 * Shared three-line card adapter (item_container_simple.xml) for the image,
 * volume, network, snapshot, profile and project lists — subclasses supply the
 * row key and the title/subtitle/detail mapping.
 */
abstract class ContainerSimpleRowAdapter<T> :
    RecyclerView.Adapter<ContainerSimpleRowAdapter<T>.ViewHolder>() {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ViewHolder {
        val view = LayoutInflater.from(parent.context)
            .inflate(R.layout.item_container_simple, parent, false)
    ...
    inner class ViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        ...
        fun bind(item: T) {
            ...
            textTitle.text = ContainerText.display(title)
            textSubtitle.text = ContainerText.display(subtitle)
            if (detail.isBlank()) {
                textDetail.visibility = View.GONE
            } else {
                textDetail.visibility = View.VISIBLE
                textDetail.text = ContainerText.display(detail)
            }
        }
    }
}
