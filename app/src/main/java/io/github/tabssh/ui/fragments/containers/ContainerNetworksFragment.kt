...
import io.github.tabssh.ui.adapters.ContainerNetworkAdapter
 ...
 */
class ContainerNetworksFragment : ContainerPageFragment() {
    ...
    private lateinit var adapter: ContainerNetworkAdapter
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        textEmpty.setText(R.string.container_networks_empty)
        view.findViewById<TextView>(R.id.text_empty_hint).setText(R.string.container_networks_empty_hint)
        view.findViewById<ImageView>(R.id.image_empty).setImageResource(R.drawable.ic_container_network)
        fabAction.contentDescription = getString(R.string.container_create_network_title)

        adapter = ContainerNetworkAdapter()
    ...
    private fun showCreateDialog() {
        ...
        MaterialAlertDialogBuilder(requireContext())
            .setTitle(R.string.container_create_network_title)
            .setView(view)
            .setPositiveButton(R.string.container_create) { _, _ ->
                ...
                } else if (!ContainerNames.isValidResourceName(name)) {
                    Toast.makeText(
                        requireContext(), R.string.container_error_name_format, Toast.LENGTH_LONG
                    ).show()
                } else if (driver.isNotEmpty() && !ContainerNames.isValidDriverName(driver)) {
                    Toast.makeText(
                        requireContext(), R.string.container_error_driver_format, Toast.LENGTH_LONG
                    ).show()
                } else {
    ...
    private fun showNetworkMenu(network: ContainerNetworkSummary) {
        if (!isAdded) return
    ...
    private fun inspectNetwork(network: ContainerNetworkSummary) {
        val current = session ?: return
        viewLifecycleOwner.lifecycleScope.launch {
            ...
            when (result) {
                is ContainerResult.Success ->
                    ContainerInspectDialog.show(requireContext(), network.name, result.value)
                else -> ContainerErrorPresenter.present(requireContext(), result)
            }
        }
    }

    private fun confirmRemove(network: ContainerNetworkSummary) {
        if (!isAdded) return
        ...
        val safeName = ContainerText.display(network.name)
        MaterialAlertDialogBuilder(requireContext())
            .setTitle(safeName)
            .setMessage(getString(R.string.container_remove_network_message, safeName))
