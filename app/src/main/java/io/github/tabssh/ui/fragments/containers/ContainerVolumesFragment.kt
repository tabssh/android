...
import io.github.tabssh.ui.adapters.ContainerVolumeAdapter
 ...
 */
class ContainerVolumesFragment : ContainerPageFragment() {
    ...
    private lateinit var adapter: ContainerVolumeAdapter
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        textEmpty.setText(R.string.container_volumes_empty)
        view.findViewById<TextView>(R.id.text_empty_hint).setText(R.string.container_volumes_empty_hint)
        view.findViewById<ImageView>(R.id.image_empty).setImageResource(R.drawable.ic_container_volume)
        fabAction.contentDescription = getString(R.string.container_create_volume_title)

        adapter = ContainerVolumeAdapter()
    ...
    private fun showCreateDialog() {
        ...
        MaterialAlertDialogBuilder(requireContext())
            .setTitle(R.string.container_create_volume_title)
            .setView(view)
    ...
    private fun showVolumeMenu(volume: ContainerVolumeSummary) {
        if (!isAdded) return
    ...
    private fun inspectVolume(volume: ContainerVolumeSummary) {
        val current = session ?: return
        viewLifecycleOwner.lifecycleScope.launch {
            ...
            when (result) {
                is ContainerResult.Success ->
                    ContainerInspectDialog.show(requireContext(), volume.name, result.value)
                else -> ContainerErrorPresenter.present(requireContext(), result)
            }
        }
    }

    private fun confirmRemove(volume: ContainerVolumeSummary) {
        if (!isAdded) return
        ...
        val safeName = ContainerText.display(volume.name)
        MaterialAlertDialogBuilder(requireContext())
            .setTitle(safeName)
            .setMessage(getString(R.string.container_remove_volume_message, safeName))
