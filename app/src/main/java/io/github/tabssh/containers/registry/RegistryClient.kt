...
open class RegistryClient(
    ...
    companion object {
        ...
        /** Cap on registry-supplied text echoed into a ContainerResult detail. */
    ...
    open suspend fun fetchManifestDigest(
        ...
        secret: String? = null
    ): ContainerResult<String> = withContext(Dispatchers.IO) {
        ...
        try {
            ...
            val authValue = when (challenge?.scheme?.lowercase()) {
                ...
                ?: secret?.takeIf { credential?.authType == "token" && it.isNotEmpty() }
                    ?.let { "Bearer $it" }
                ?: return@withContext ContainerResult.PermissionDenied(
            ...
            ContainerResult.Error("Registry check failed for ${ref.canonicalRepository}", e.message)
        }
    }
     ...
     */
    private fun digestFrom(response: Response, ref: ImageRef): ContainerResult<String> {
        ...
        response.header("Docker-Content-Digest")?.trim()?.takeIf { ImageRef.isValidDigest(it) }?.let {
            return ContainerResult.Success(it)
        }
        ...
        http.newCall(get).execute().use { body ->
            ...
            val bytes = readBoundedBytes(body)
                ?: return ContainerResult.Error("Manifest body was empty or oversized for ${ref.canonicalRepository}")
    ...
    /** Map a registry HTTP failure onto the shared ContainerResult failures. */
    private fun classify(response: Response, ref: ImageRef): ContainerResult<Nothing> = when (response.code) {
        404 -> ContainerResult.NotFound(
