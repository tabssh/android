...
object DockerApiParsers {
    ...
    /** `GET /version` body. */
    fun parseVersion(body: String): ContainerEngineVersion? {
        return try {
    ...
    fun parseContainerList(body: String): List<ContainerSummary> {
        val arr = JSONArray(body)
        val result = mutableListOf<ContainerSummary>()
        for (i in 0 until arr.length()) {
            ...
            result += ContainerSummary(
                ...
                labels = parseStringMap(obj.optJSONObject("Labels"))
            )
        }
        return result
    }

    /** Flatten a JSON string-map ("Labels") to a Kotlin map; null → empty. */
    ...
    fun parseImageList(body: String): List<ContainerImageSummary> {
        val arr = JSONArray(body)
        val result = mutableListOf<ContainerImageSummary>()
        for (i in 0 until arr.length()) {
            val obj = arr.getJSONObject(i)
    ...
    fun parseVolumeList(body: String): List<ContainerVolumeSummary> {
        val arr = JSONObject(body).optJSONArray("Volumes") ?: return emptyList()
        val result = mutableListOf<ContainerVolumeSummary>()
        for (i in 0 until arr.length()) {
            val obj = arr.getJSONObject(i)
            result += ContainerVolumeSummary(
    ...
    fun parseNetworkList(body: String): List<ContainerNetworkSummary> {
        val arr = JSONArray(body)
        val result = mutableListOf<ContainerNetworkSummary>()
        for (i in 0 until arr.length()) {
            val obj = arr.getJSONObject(i)
            result += ContainerNetworkSummary(
                id = obj.optString("Id"),
                name = obj.optString("Name"),
                driver = obj.optString("Driver"),
    ...
    /** `GET /info` body. */
    fun parseInfo(body: String): ContainerEngineInfo? {
        return try {
            val obj = JSONObject(body)
            ContainerEngineInfo(
                name = obj.optString("Name"),
     ...
     */
    fun parseSystemDf(body: String): ContainerDiskUsage {
        val obj = JSONObject(body)
        ...
        return ContainerDiskUsage(rows)
    }
     ...
     */
    fun parseApiStats(body: String): ContainerStats? {
        return try {
            val obj = JSONObject(body)
