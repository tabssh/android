...
class UpdateChecker(
    ...
    suspend fun checkAll(hostId: Long, transport: ContainerTransport): List<CheckResult> =
        policyDao.getEnabledList()
            .filter { it.containerHostId == hostId }
    ...
    suspend fun checkOne(
        ...
        val inspect = when (val r = transport.inspectContainer(policy.containerNameOrStackName)) {
            is ContainerResult.Success -> normalizeInspect(r.value)
        ...
        val registryDigest =
            when (val r = registry.fetchManifestDigest(ref, credential, secret)) {
                is ContainerResult.Success -> r.value
    ...
    private suspend fun resolveRunningDigest(
        ...
        val imageInspect = when (val r = transport.inspectImage(target)) {
            is ContainerResult.Success -> normalizeInspect(r.value) ?: return null
            else -> return null
        }
